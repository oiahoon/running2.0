...
- Favorite-runner visuals are generated low-poly character cutouts under `apps/web/public/assets/runners/`. They can appear as lively cameos across dashboard/poster surfaces, but each cameo needs reserved space or explicit text padding so the full silhouette remains visible and does not cover route glyphs, metric rows, hero text, or primary controls.
